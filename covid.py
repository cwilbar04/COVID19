...
def load_covid_data():
    ...
    Returns
   -------
    Transformed dataframe based on county level data loaded from https://raw.githubusercontent.com/nytimes/covid-19-data/master/us-counties.csv
    
    See https://github.com/nytimes/covid-19-data/blob/master/README.md for more info on source dataset.
    """
    ...
    state_level_data = pd.read_csv('./data/state_level_fips_codes.csv')
    ...
    # Load file with relative population size of counties for the covid entries with missing fips codes
    # Data for the cities is broken in to the respective counties with a column indicating the relative size of the county compared
    # to other counties in the same city
    # This is still inaccurate but at least it does not eliminate the data from these large cities when looking at fips level data
    # This data is rounded down to minimize the impact
    population_df = pd.read_csv('./data/missing_fips_population_proportion.csv', sep='|', header=True, index=False)
    
    
    # Merge population proportions to main dataframe on FIPS to attach population proportion amounts to Missouri Counties that are part 
    # of a city reported separately
    covid = covid.merge(population_df[['City,State','FIPS','County Population Proportion']], how='left', left_on='fips', right_on='FIPS')

    # Merge covid dataframe to itself on "City,State" to "county,state" and date to add the cases and deaths for the cities in Missouri
    ...
    # For New York City, merge population dataframe on "county,state" to "City,State" to add new rows for each county in New York City
    ...
    covid = covid.merge(population_df.loc[population_df['City,State'] == 'New York City,New York',
                                            ['City,State','FIPS','County','County Population Proportion']], 
    ...
    # Cases/deaths will be lost but not as many as if we did not perform any of these transformations
    ...
    covid.loc[filter_condition,'cases'] = (covid.loc[filter_condition,'cases']*covid.loc[filter_condition,'County Population Proportion']).astype(int)
    covid.loc[filter_condition,'deaths'] = (covid.loc[filter_condition,'deaths']*covid.loc[filter_condition,'County Population Proportion']).astype(int)

    # Remove columns only added for calculations
    covid.drop(labels=['City,State','FIPS','County Population Proportion', 'County'], axis = 1, inplace=True)
    ...
    # Note there is a known inconsistency in data that cumulative totals do not always increase if data is updated on a previous date.
    # This can result is negative values for daily cases or deaths. For now these have been left as is so changes can be understand and aggregations
    # wil be consistent
    # Consider changing this logic to set these values to 0 instead of negative values depending on use case and target audience
